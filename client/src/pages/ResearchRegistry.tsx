...
export default function ResearchRegistry() {
  ...
  return <DashboardLayout><main className="space-y-6 p-6">
    <header><h1 className="text-2xl font-semibold">Research registry</h1><p className="text-sm text-muted-foreground">Owner-entered, research-only hypotheses. No registry status authorizes live trading.</p><p className="mt-1 text-xs text-muted-foreground">Moving to validated requires explicit owner confirmation plus a protocol reference, result reference, and adequate recorded sample. This is an evidence label, not a trading approval.</p>{list.error && <p className="mt-2 text-sm text-destructive">The registry could not be loaded. Refresh the page and verify that you are signed in before retrying.</p>}</header>
