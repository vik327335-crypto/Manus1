...
const PERIOD = { start: "2021-08-11", end: "2022-08-10" };
const PARAMETERS = {
  signalType: "momentum",
  fastPeriod: null,
  slowPeriod: null,
...
async function main() {
  ...
  const targetMet = aggregateProfitFactor !== null && aggregateProfitFactor >= 1.5 && completedTrades >= 9 && positiveReturnAssets >= 2;
  ...
  console.log(JSON.stringify({
    ...
    protocol: "Parameters were selected on BTC/USDT pre-test periods only; ETH/USDT and SOL/USDT were not used to select or alter them.",
    period: PERIOD,
    execution: "Entries and signal exits are executed at the next daily open from indicators using only preceding completed daily closes. Stop-loss has precedence over take-profit when both levels are reached intraday.",
    assumptions: { feeRatePerSide: 0.001, slippageBeyondFee: "not modelled", equalInitialCapitalPerAsset: 100000, minimumAggregateTrades: 9, minimumPositiveReturnAssets: 2 },
