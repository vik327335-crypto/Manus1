...
const UNIVERSE = ["BTCUSDT", "ETHUSDT", "SOLUSDT", "BNBUSDT", "XRPUSDT", "ADAUSDT"];
const PARAMETERS = {
  signalType: "momentum",
  fastPeriod: null,
  slowPeriod: null,
...
async function main() {
  ...
  const profitContribution = results.map((result) => ({ symbol: result.symbol, share: grossProfit === 0 ? 0 : result.metrics.grossProfit / grossProfit }));
  const profitConcentrationHhi = profitContribution.reduce((sum, item) => sum + item.share ** 2, 0);
  const largestProfitContributor = profitContribution.reduce((largest, item) => item.share > largest.share ? item : largest, { symbol: "", share: 0 });
  const targetMet = aggregateProfitFactor !== null && aggregateProfitFactor >= 1.5 && completedTrades >= 18 && positiveReturnAssets >= 3;
  ...
  console.log(JSON.stringify({
    ...
    protocol: "The six-asset universe was declared before this evaluation. Strategy parameters were selected before 2024-08-12 and are not changed here.",
    period,
    ...
    assumptions: { feeRatePerSide: 0.001, slippageBeyondFee: "not modelled", equalInitialCapitalPerAsset: 100000, assetWeight: 1 / UNIVERSE.length, minimumAggregateTrades: 18, minimumPositiveReturnAssets: 3 },
    universe: UNIVERSE,
    parameters: PARAMETERS,
    results: results.map((result) => ({ symbol: result.symbol, metrics: metrics(result), profitContribution: round(profitContribution.find((item) => item.symbol === result.symbol).share), trades: result.trades.map((trade) => ({ ...trade, pnl: round(trade.pnl), pnlPercent: round(trade.pnlPercent) })) })),
    aggregate: { completedTrades, grossProfit: round(grossProfit), grossLoss: round(grossLoss), profitFactor: aggregateProfitFactor === null ? null : round(aggregateProfitFactor), positiveReturnAssets, profitConcentrationHhi: round(profitConcentrationHhi), largestProfitContributor: { symbol: largestProfitContributor.symbol, share: round(largestProfitContributor.share) }, targetMet },
  }, null, 2));
}
